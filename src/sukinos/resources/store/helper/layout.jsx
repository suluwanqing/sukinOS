...
async def sukinOS_upload(
    ...
    try:
        ...
        db.add(D_SukinosApp(**D_sukinos_App(**data).model_dump()))
        db.commit()
...
async def get_app_list(
    ...
    try:
        ...
        total_count = db.query(D_SukinosApp).count()
        datas = db.query(D_SukinosApp).offset(offset).limit(pageSize).all()
        result = {
            "items": datas,
            "pagination": {
...
async def check_updates(request: Request, db: Session = Depends(get_db)):
    try:
        ...
        for item in apps_to_check:
            ...
            db_app = db.query(D_SukinosApp).filter(D_SukinosApp.resource_id == r_id).first()
            if db_app:
                # 尝试浮点数比对，失败则回退到字符串比对
                try:
...
async def search_app(keyword: str = Query(...), db: Session = Depends(get_db)):
    try:
        data = db.query(D_SukinosApp).filter(D_SukinosApp.app_name.like(keyword)).all()
...
async def user_uploads(user: User = Depends(verify_auth()), db: Session = Depends(get_db)):
    try:
        data = db.query(D_SukinosApp).filter_by(user_id=user.id).all()
        return api_response(code=200, msg="获取成功", data=data)
    except Exception as e:
        return api_response(code=500, msg="服务器错误: " + str(e))
...
async def user_delete(
    ...
    try:
        data = db.query(D_SukinosApp).filter_by(user_id=user.id, resource_id=resourceId).first()
        if data:
            db.delete(data)
            db.commit()
