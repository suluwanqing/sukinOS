...
const AppPanel = ({
  ...
  <div className={style[bem.e('panel-body')]}>
    ...
    <div className={style[bem.e('grid')]}>
      ...
      <Field label="图标路径">
        <Input value={appMeta.appIcon} onChange={e => updateAppMeta({ appIcon: e.target.value })} placeholder={DEFAULT_LOGO} size="medium" />
      </Field>
    ...
    <Divider />
    <Cap>更新已上传</Cap>
    <div className={style[bem.e('update-actions')]}>
      <button className={style[bem.e('btn-ghost')]} onClick={onRefreshUploads}>刷新列表</button>
      <button className={style[bem.e('btn-solid')]} onClick={onUpdateSelected} disabled={isUpdating || !canUpdate}>更新已选</button>
    </div>
    <div className={style[bem.e('update-list')]}>
      {myUploads.length === 0 ? (
        <div className={style[bem.e('update-empty')]}>暂无可更新应用</div>
      ) : (
        myUploads.map(app => (
          <label key={app.resourceId} className={style[bem.e('update-item')]}>
            <input
              type="checkbox"
              checked={selectedUploadIds.has(app.resourceId)}
              onChange={() => onToggleUploadSelect(app.resourceId)}
            />
            <span className={style[bem.e('update-name')]}>{app.appName}</span>
            <span className={style[bem.e('update-version')]}>v{app.version}</span>
          </label>
        ))
      )}
    </div>
    {updateMsg && (
      <div className={[style[bem.e('update-msg')], updateMsgType === 'error' ? style[bem.e('update-msg--err')] : style[bem.e('update-msg--ok')]].join(' ')}>
        {updateMsg}
      </div>
    )}
  </div>
...
const LocalDev = () => {
  ...
  const canUpdate = fetchStatus === 'ok' && hasLogic && missingRequired.length === 0;

  const toggleUploadSelect = (resourceId) => {
  ...
  const handleUpdateSelected = async () => {
    if (!canUpdate) {
      setUpdateMsg('请先完成连接与初始化');
      setUpdateMsgType('error');
      return;
    }
    if (!appMeta.sysOptions?.shouldUpload) {
      setUpdateMsg('请先开启云端上传');
      setUpdateMsgType('error');
      return;
    }
    if (selectedUploadIds.size === 0) {
      setUpdateMsg('请先选择需要更新的应用');
      setUpdateMsgType('error');
      return;
    }
    if (missingRequired.length > 0) {
      setUpdateMsg(`缺少必要文件：${missingRequired.join('、')}`);
      setUpdateMsgType('error');
      return;
    }
    ...
    confirm.show({
      title: '确认更新',
      content: '将使用当前本地预览内容覆盖云端版本，是否继续？',
      onConfirm: async () => {
        setIsUpdating(true);
        setUpdateMsg('');
        try {
          ...
          const cleanFiles = {};
          for (const [k, v] of Object.entries(localFiles)) { cleanFiles[k.replace(/\.(jsx|js)$/, '')] = v; }
          const targets = myUploads.filter(app => selectedUploadIds.has(app.resourceId));
      ...
      <div className={[style[bem.e('drawer')], drawerOpen ? style[bem.e('drawer--open')] : ''].join(' ')} ref={drawerRef}>
        ...
        <div className={style[bem.e('drawer-scroll')]}>
          ...
          {activeNav === 'app' && (
            <AppPanel
              appMeta={appMeta}
              updateAppMeta={updateAppMeta}
              isManualType={isManualType}
              setIsManualType={setIsManualType}
              myUploads={myUploads}
              selectedUploadIds={selectedUploadIds}
              onToggleUploadSelect={toggleUploadSelect}
              onUpdateSelected={handleUpdateSelected}
              onRefreshUploads={refreshMyUploads}
              updateMsg={updateMsg}
