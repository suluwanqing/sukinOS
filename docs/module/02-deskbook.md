...
| `generateAppSetting` | `object` | 全局应用生成设置（singleIframe, useVirtualWorker） |
| `ref` | `Ref` | DOM 引用回传 |
...
`memoEqual` 对 13 个关键 props 进行浅比较，确保只有在实际相关变化时才重渲染。
...
- 左侧：偏好设置开关列表（通过 `appCustomMapper` 配置）
