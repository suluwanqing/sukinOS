...
- 渲染 `null`，无 UI 效果
- 挂载时从 API 拉取最新路由权限配置写入 Redux
- 覆盖 redux-persist 恢复的可能过时数据
- 与 axios 请求拦截器联动，本地拦截越权请求

**读取层**: `src/middleware/routePermission/cache.js`

```javascript
checkRoutePermission(method, path): boolean
// - 从 Redux store.getState() 实时读取，无本地可变状态
// - 配置未加载 → 放行
// - 未登录 → 拦截
// - root → 放行
// - 路由不在配置中 → 放行
...
**位置**: `src/middleware/routePermission/`

RoutePermission 是后端配置驱动的 API 路由权限检查：
- 用户登录后 `useAuth` 从 `/api/system/permission/routes-permission` 加载权限配置表写入 Redux
- 页面挂载时 bridge 组件重新拉取覆盖 persist 旧数据
- `checkRoutePermission(method, path)` 通过 `store.getState()` 实时读取 Redux 判断
